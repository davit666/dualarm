...
def draw_sphere_body(position, radius, rgba_color):
    ...
    return body_id
def draw_cube_body(position, orientation, halfExtents, color):
    vs_id = p.createVisualShape(p.GEOM_BOX, halfExtents=halfExtents, rgbaColor=color)
    object_id = p.createMultiBody(basePosition=position, baseOrientation=orientation, baseCollisionShapeIndex=-1,
...
def create_cylinder_object(position, radius=0.1, height=0.3, color=[0, 1, 0, 1]):
    vs_id = p.createVisualShape(p.GEOM_CYLINDER, radius=radius, length=height, rgbaColor=color)
    coll_id = p.createCollisionShape(p.GEOM_CYLINDER, radius=radius, height=height)
    object_id = p.createMultiBody(basePosition=position, baseCollisionShapeIndex=coll_id,
                                  baseVisualShapeIndex=vs_id)
    return object_id
...
def remove_obstacles(obstacle_ids):
    for obstacle_id in obstacle_ids:
        p.removeBody(obstacle_id)

def contact_point_check(obj1_id, obj2_id):
    contact_point_list = p.getContactPoints(obj1_id, obj2_id)
    return contact_point_list


def closest_point_check(obj1_id, obj2_id, dist=0.02):
    close_point_list = p.getClosestPoints(obj1_id, obj2_id, dist)
    return close_point_list